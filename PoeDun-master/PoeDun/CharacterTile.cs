...
namespace PoeDun
{
    public abstract class CharacterTile : Tile // extends the Tile class
    {
        ...
        private Tile[] vision; // the vision array will store the character's vision
        ...
        public CharacterTile(Position posTile) : base(posTile)
        {
        }

        // property that will check if the character is dead
        public bool IsDead
        {
            get
            {
        ...
        public void UpdateVision(Level level)
        {
            ...
            vision[0] = level.Tiles[x, y - 1]; // tile above
            vision[1] = level.Tiles[x + 1, y]; // tile to the right
            vision[2] = level.Tiles[x, y + 1]; // tile below
            vision[3] = level.Tiles[x - 1, y]; // tile to the left
        }
